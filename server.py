...
class SIPRegisterHandler(socketserver.DatagramRequestHandler):
    ...
    def register2json(self):
        """Este metodo crea un fichero .json y escribe los datos en él."""
    ...
    def json2registered(self):
        """
        En este metodo comprobamos si ya existe un fihero .json para seguir.
        escribiendo dentro de él.
        """
        try:
    ...
    def expired_users(self):
        """
        Comprobamos para la lista de usuarios registrados, si existe alguno.
        que haya expirado y lo añadimos a una lista de usuarios expirados para.
        mas tardes eliminarlos de nuestro registro.
        """
        exp_user = []
        ...
        if contenido[0] == "REGISTER":
            ...
            if contenido[-2] == "Expires:":
                expires = time.strftime('%Y-%m-%d %H:%M:%S',
                                        time.gmtime(time.time() +
                                                    int(contenido[-1])))
                self.registro[usuario] = [{"address": ip}, {"expires":
                                                            expires}]
        self.expired_users()
        self.register2json()
